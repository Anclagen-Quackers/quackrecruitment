...
const CTASection = ({
  ...
  return (
    <div
      className={`flex relative transition-all duration-2000 transform ${inView ? "" : "opacity-0 mt-20"}`}
      ref={ref}
    >
      <div
        className={`text-start lg:w-1/2 text-center py-12 px-4 sm:px-6 lg:py-16 lg:px-8 z-20 ${reverse ? "md:order-2" : "md:order-1"}`}
      >
      ...
      <div className={`lg:w-1/2 ${reverse ? "md:order-1" : "md:order-2"}`}>
        <img
          src={image}
          className="top-0 right-0 hidden h-full max-w-1/2 lg:block p-5 mx-auto"
          alt={alt}
        />
      </div>
    </div>
